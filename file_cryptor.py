# Python 3.10.6
# pycryptodomex 3.16.0
# mnemonic 0.20

from os import system
from os import listdir
from base64 import b64encode
from base64 import b64decode
from getpass import getpass
import sys
import secrets
import readline

# pycryptodomex
from Cryptodome.Cipher import AES
from Cryptodome.Random import get_random_bytes

# mnemonic
from mnemonic import Mnemonic

KEY_DEFAULT = '7ebfbdb5950ebb7c46d111962f47b4a11165bd9de79a9c1dd0426f9dab67856f'

class DataCryptor():
    @staticmethod
    def encrypt(key:bytes, data:bytes) -> bytes:
        header = get_random_bytes(16)
        cipher = AES.new(key, AES.MODE_EAX)
        cipher.update(header)
        ciphertext, tag = cipher.encrypt_and_digest(data)
        return header + cipher.nonce + tag + ciphertext

    @staticmethod
    def decrypt(key:bytes, data:bytes) -> bytes:
        header = data[0:16]
        nonce = data[16:32]
        tag = data[32:48]
        ciphertext = data[48:]
        cipher = AES.new(key, AES.MODE_EAX, nonce=nonce)
        cipher.update(header)
        plaintext = cipher.decrypt_and_verify(ciphertext, tag)
        return plaintext

class FileCryptor():
    @staticmethod
    def encrypt(key:bytes, file_path):
        with open(file_path, 'rb') as f:
            data = f.read()
        ciphertext = DataCryptor.encrypt(key, data)
        with open(file_path, 'w') as f:
            f.write(b64encode(ciphertext).decode('utf-8'))
    
    @staticmethod
    def decrypt(key:bytes, file_path):
        with open(file_path, 'r') as f:
            data = f.read()
        ciphertext = b64decode((data.strip()).encode('utf-8'))
        plaintext = DataCryptor.decrypt(key, ciphertext)
        with open(file_path, 'wb') as f:
            f.write(plaintext)


if __name__ == "__main__":
    system('clear')
    brief_introduction = 'Casper File Cryptor v3.0.2'
    print(brief_introduction)
    print('*********************************')

    mnemo = Mnemonic('english')
    while True:
        option = input('[menu] 1.Input key  2.Create key  3.Quit >> ')

        system('clear')
        print(brief_introduction)
        print('*********************************')
        
        # Input key
        if option in ['i', 'I', 'input', 'INPUT']:
            word = input('Mnemonic: ')

            if word == '':
                if (input('[message] Use default key? [Y/n] >> ') or 'n') == 'Y':
                    key = bytes.fromhex(KEY_DEFAULT)
                    key_tip = '[Message] Using default key'
                    break
                else:
                    continue

            try:
                key = (bytes(mnemo.to_entropy(word)) + bytes.fromhex(KEY_DEFAULT))[0:32]
            except Exception as e:
                print('[warning] Key format error')
                continue
            
            try:
                ciphertext = 'bQbAK/rA6MZj4cRxRj9aBC7MImDrUfcgGBb/uBx45wUqN7XaSA/OFVfIl3V1CTmgnrTMl/8b7asET5eihaF+MsairncjcmsbkA'
                plaintext = DataCryptor.decrypt(key, b64decode(ciphertext.encode('utf-8')))
                key_tip = '[Message] ' + plaintext.decode('utf-8')
                break
            except Exception as e:
                if (input('[message] Use this unknow key? [Y/n] >> ') or 'n') == 'Y':
                    key_tip = '[Warning] Using unknow key'
                    break
                else:
                    continue

        # Creat key
        elif option in ['c', 'C', 'creat', 'CREAT']: 
            key_len = input('Key length (default 32 bytes): ') or '32'
    
            new_key = secrets.token_bytes(int(key_len))

            print('New key: {0}'.format(new_key))
            print('Hexadecimal: ' + new_key.hex())
            print('Base64: ' + b64encode(new_key).decode('utf-8'))
            if int(key_len) not in [16, 20, 24, 32]:
                print('Mnemonic: None' + '\n')
            else:
                mnemonic = mnemo.to_mnemonic(new_key)
                print('Mnemonic: ' + mnemonic + '\n')

        # Quit
        elif option in ['q', 'Q', 'quit', 'QUIT']:
            system('clear')
            sys.exit()

        # Invalid option
        else:
            print('[Message] Invalid option' + '\n')

    while True:
        option = input('[menu] 1.Encrypt file  2.Decrypt file  3.Show current key  4.Quit >> ')

        system('clear')
        print(brief_introduction)
        print('*********************************')
        
        # Encrypt file
        if option in ['e', 'E', 'encrypt','ENCRYPT']:
            print('List: ' + '  '.join(listdir('./')))
            file_name = input('File: ')
            try:
                FileCryptor.encrypt(key, file_name)
                print('[Message] Encryption successful' + '\n')
            except Exception as e:
                print('[Warning] Encryption failed' + '\n')
        
        # Decrypt file
        elif option in ['d', 'D', 'decrypt', 'DECRYPT']:
            print('List: ' + '  '.join(listdir('./')))
            file_name = input('File: ')
            try:
                FileCryptor.decrypt(key, file_name)
                print('[Message] Decryption successful' + '\n')
            except Exception as e:
                print('[Warning] Decryption failed' + '\n')
        
        # Show current key
        elif option in ['s', 'S', 'show', 'SHOW']:
            key = input('Mnemonic: ') or key
            print('Current key: {0}'.format(key))
            print('Hexadecimal: ' + key.hex())
            print('Base64: ' + b64encode(key).decode('utf-8'))
            print('Mnemonic: ' + mnemo.to_mnemonic(key) + '\n')
            print(key_tip)

        # Quit
        elif option in ['q', 'Q', 'quit', 'QUIT']:
            system('clear')
            sys.exit()

        # Invalid option
        else:
            print('[Message] Invalid option' + '\n')
